...
- Dashboard data ships as normalized JSON in `data/data-all.json` (with an optional `data/data-exclude-p266.json` fallback). The loader still accepts legacy aggregate JSON if you need it, but it prefers the normalized payloads produced by the ingestion script.
...
## Refreshing datasets from the Excel workbook

The dashboard reads normalized payloads from `data/data-all.json` (default) and, optionally, `data/data-exclude-p266.json` for legacy/API use. Run the ingestion script whenever new participant data lands.
...
3. Generate staged outputs so you can diff before promoting them:
   ```bash
   python3 scripts/ingest_dashboard_data.py \
     ...
     --exclude p266
   ```
   - `--sheet` defaults to `merged_data-all`; change it if the worksheet is named differently.
   - `--exclude` emits a trimmed variant alongside the full dataset. Skip it if you only need the full set—the participant filter can exclude IDs like p266 in the UI.
   - You can invoke the script via npm as well:  
     `npm run ingest-data -- --source "docs/SUDCare Dashboard v0 Dataset.xlsx" --output data/normalized --exclude p266 --dataset-prefix sudcare`
4. Promote the refreshed files:
   ```bash
   cp data/normalized/sudcare_all.json data/data-all.json
   cp data/normalized/sudcare_exclude.json data/data-exclude-p266.json # optional
   ```
   - If you prefer, set `--output data` to write directly to the live files (overwrites in-place).
...
- `dataset` is a short slug that matches the `DatasetKey` union used by the loader (default `all` on the home page).
...
3. The home page currently boots the `all` dataset in `src/app/page.tsx`. If you add another dataset, surface a selector (or change that default) so it is reachable, and update the labels/descriptions in `dashboard-data-provider.tsx` as needed.
